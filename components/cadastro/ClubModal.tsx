...
interface Props { open: boolean; editing: Club | null; leagues: League[]; onClose: () => void; onSave: (form: ClubForm) => void; saving: boolean }
...
function toForm(c: Club): ClubForm { return { league_id: c.league_id, name: c.name, external_id: c.external_id, settlement_type: c.settlement_type, moeda: c.moeda, taxa_tipo: c.taxa_tipo, fee_mtt_pct: c.fee_mtt_pct, fee_cash_pct: c.fee_cash_pct, taxa_op_pct: c.taxa_op_pct, taxa_op_tipo: c.taxa_op_tipo, spinup_pct: c.spinup_pct, rebate_pct: c.rebate_pct, crypto_rebate_pct: c.crypto_rebate_pct, rakeback_pct: c.rakeback_pct, security: c.security, taxa_variavel_nome: c.taxa_variavel_nome, taxa_variavel_indicador: c.taxa_variavel_indicador, taxa_variavel_regra: c.taxa_variavel_regra, caucao_atual: c.caucao_atual, stoploss_inicial: c.stoploss_inicial } }
...
export function ClubModal({ open, editing, leagues, onClose, onSave, saving }: Props) {
  ...
  useEffect(() => { setForm(editing ? toForm(editing) : EMPTY); setAdv(false); setIndicacoes([]); setIndClub(''); setIndTaxa('') }, [editing, open])
  if (!open) return null
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            <Sec title="Identificação">
              <div className="grid grid-cols-2 gap-4">
                ...
                <Fld label="ID no App"><input type="text" value={form.external_id ?? ''} onChange={e => set('external_id', e.target.value || null)} placeholder="Ex: 1548056" className={inputCls} /></Fld>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Fld label="Liga" required>
                  <select value={form.league_id ?? ''} onChange={e => set('league_id', e.target.value || null)} required className={inputCls}>
            ...
            <Sec title="Modelo de Acerto">
              <Fld label="Tipo de Settlement">
                <div className="grid grid-cols-3 gap-2">
                  {[{v:'dinamico',l:'Taxa Dinâmica',d:'Ligas LP'},{v:'weekly_usd',l:'Weekly USD',d:'Ligas ORION'},{v:'rakeback',l:'Rakeback',d:'Ligas SUL_HG'}].map(o => (
                    <button key={o.v} type="button" onClick={() => set('settlement_type', o.v)} className={`p-3 rounded-lg border text-left transition-all ${form.settlement_type === o.v ? 'border-gold bg-gold/10 text-white' : 'border-white/10 text-gray-400 hover:border-white/20'}`}>
                      <div className="text-sm font-medium">{o.l}</div>
                      <div className="text-xs mt-0.5 opacity-60">{o.d}</div>
                    </button>
                  ))}
                </div>
              </Fld>
            ...
            <Sec title="Taxas">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Fld label="Taxa Operacional App (%)"><NumInput value={form.taxa_op_pct} onChange={v => set('taxa_op_pct', v)} placeholder="Ex: 9" /></Fld>
                <Fld label="Security (fixo semanal)"><NumInput value={form.security} onChange={v => set('security', v)} placeholder="Ex: 500" /></Fld>
              </div>
              {!isRkb && (
                <div className="grid grid-cols-2 gap-4">
                  <Fld label="Rebate (%)"><NumInput value={form.rebate_pct} onChange={v => set('rebate_pct', v)} placeholder="Ex: 10" /></Fld>
