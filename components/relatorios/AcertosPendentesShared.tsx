...
import type { LinhaInadimplencia, StatusStoploss } from '@/lib/acertos-pendentes'
...
export function StatusBadge({ status }: { status: StatusStoploss }) {
  const { t } = useI18n()
...
export function TabelaInadimplencia({ titulo, linhas, vazio, mostrarTaxa }: { titulo: string; linhas: LinhaInadimplencia[]; vazio: string; mostrarTaxa?: boolean }) {
  const { t } = useI18n()
