...
import { buscarAcertosPendentesDaSemana, buscarClubesComDividasAntigas, type LinhaAcertoPendenteSemana, type LinhaDividaAntiga, type StatusStoploss } from '@/lib/acertos-pendentes'
...
export function RelatorioAcertosPendentes() {
  ...
  const [dividas, setDividas] = useState<LinhaDividaAntiga[]>([])
  ...
  useEffect(() => {
    ...
    Promise.all([buscarAcertosPendentesDaSemana(), buscarClubesComDividasAntigas()])
      .then(([s, d]) => { setSemana(s); setDividas(d) })
  ...
  const totalDividas = dividas.reduce((acc, l) => ({ acerto: acc.acerto + l.acerto, pago: acc.pago + l.pago, diferenca: acc.diferenca + l.diferenca }), { acerto: 0, pago: 0, diferenca: 0 })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <h2 className="text-lg font-semibold text-white">{t('acertos_pendentes.dividas_titulo')}</h2>
        <div className="rounded-xl border border-white/10 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {dividas.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500 text-sm">{t('acertos_pendentes.nenhuma_divida_antiga')}</td></tr>
                ) : (
                  dividas.map((l) => (
              ...
              {dividas.length > 0 && (
                <tfoot>
                  <tr className="border-t border-white/10 bg-surface2">
                    ...
                    <td className="px-4 py-3 text-right text-gray-300 font-semibold">{fmt(totalDividas.acerto)}</td>
                    <td className="px-4 py-3 text-right text-gray-300 font-semibold">{fmt(totalDividas.pago)}</td>
                    <td className="px-4 py-3 text-right text-alert font-semibold">{fmt(totalDividas.diferenca)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
