    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface border border-white/10 rounded-2xl w-full max-w-sm mx-4 shadow-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-white">{t('confirm_cotacao_modal.titulo', { clube: clube.name })}</h2>
        </div>

        <div className="space-y-3">
          <label className="block text-sm font-medium text-gray-300">{t('confirm_cotacao_modal.pergunta')}</label>
          <input
            type="text"
            inputMode="decimal"
            autoFocus
            ...
            placeholder={t('confirm_cotacao_modal.placeholder')}
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            <button onClick={salvar} disabled={saving} className="flex items-center gap-2 px-5 py-2 bg-gold text-surface rounded-lg text-sm font-semibold hover:bg-gold/90 disabled:opacity-50 transition-colors">
              {saving && <Loader2 size={14} className="animate-spin" />}{t('confirm_cotacao_modal.salvar')}
            </button>
          </div>
        </div>
      </div>
