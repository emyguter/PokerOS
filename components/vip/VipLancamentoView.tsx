...
interface ClubeOpcao extends LimitesVipClube { id: string; name: string; external_id: string | null }
...
export function VipLancamentoView() {
  const { t } = useI18n()
  ...
  useEffect(() => {
    supabase.from('clubs').select('id, name, external_id, limite_vip_silver, limite_vip_black, limite_vip_platinum').order('name').then(({ data }) => setClubes(data ?? []))
  }, [])

  const clubesPorId = useMemo(() => new Map(clubes.map((c) => [c.id, c])), [clubes])
  const clubeSelecionado = clubesPorId.get(clubeId)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const limite = clubeSelecionado ? limiteVipDoClube(clubeSelecionado, tipo) : 0
    if (atual >= limite) {
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {lancamentos.map((l) => {
                  ...
                  const clube = clubesPorId.get(l.clube_id)
                  const limite = clube ? limiteVipDoClube(clube, l.tipo) : 0
                  const cor = corVip(usados, limite)
                  return (
                    <tr key={l.id} className={`border-b border-white/5 transition-colors ${ROW_COR[cor]}`}>
                      ...
                      <td className={`px-4 py-3 font-medium ${BADGE_COR[cor]}`}>{usados}/{limite || '—'}</td>
