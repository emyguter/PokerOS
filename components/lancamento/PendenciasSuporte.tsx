...
// Pendência do Suporte: a Genia (Financeiro) lançou algo que o Suporte não
// tem registro correspondente — falta ele conferir/lançar.
export function PendenciasSuporte() {
  const { t } = useI18n()
  const { dataInicio, setDataInicio, dataFim, setDataFim, loading, error, pendSuporte, suporteEntradas, salvarValor, vincular } = useConciliacao()
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-500">{t('conciliacao.pendencias_suporte_desc')}</p>
      ...
      {loading ? (
        ...
        <ListaPendencias itens={pendSuporte} ladoOposto={suporteEntradas} vazio={t('conciliacao.sem_pendencias')} onSalvarValor={salvarValor} onVincular={vincular} />
      )}
    </div>
  )
}
