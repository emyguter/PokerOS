    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface border border-white/10 rounded-2xl w-full max-w-md mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
          <h2 className="text-lg font-semibold text-white">{t('interromper_modal.titulo')}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
            <p className="text-xs text-gray-500">
              {t('interromper_modal.desc_pre')} <span className="text-white">{divida.clube_nome}</span> {t('interromper_modal.desc_pos')}
            </p>
            <div className="rounded-lg border border-gold/30 bg-gold/5 p-3">
              <p className="text-xs text-gray-400">{t('interromper_modal.saldo_renegociar')}</p>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                {pagoComRake ? t('interromper_modal.pagar_com_rake_on') : t('interromper_modal.pagar_com_rake_off')}
              </p>
            </div>
          ...
          <div className="shrink-0 flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10">
            ...
            <button type="submit" disabled={saving || !podeSalvar} className="flex items-center gap-2 px-5 py-2 bg-gold text-surface rounded-lg text-sm font-semibold hover:bg-gold/90 disabled:opacity-50 transition-colors">
              {saving && <Loader2 size={14} className="animate-spin" />}{t('interromper_modal.confirmar')}
            </button>
          </div>
        </form>
      </div>
