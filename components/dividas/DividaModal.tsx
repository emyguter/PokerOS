    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface border border-white/10 rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
          <h2 className="text-lg font-semibold text-white">{editing ? t('divida_modal.editar_titulo') : t('divida_modal.novo_titulo')}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.clube')}<span className="text-gray-500 ml-1">*</span></label>
              {editing ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" disabled={!!editing} onClick={() => setTipo('simples')} className={`px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed ${tipo === 'simples' ? 'border-gold/50 bg-gold/5 text-white' : 'border-white/10 text-gray-400 hover:border-white/20'}`}>
                {t('divida_modal.simples_titulo')}
                <p className="text-xs font-normal text-gray-500 mt-0.5">{t('divida_modal.simples_desc')}</p>
              </button>
              <button type="button" disabled={!!editing} onClick={() => setTipo('acordo')} className={`px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed ${tipo === 'acordo' ? 'border-gold/50 bg-gold/5 text-white' : 'border-white/10 text-gray-400 hover:border-white/20'}`}>
                {t('divida_modal.acordo_titulo')}
                <p className="text-xs font-normal text-gray-500 mt-0.5">{t('divida_modal.acordo_desc')}</p>
              </button>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.valor_integral')}<span className="text-gray-500 ml-1">*</span></label>
              {travado ? (
                <input type="text" value={fmt(Number(valorIntegral.replace(',', '.')) || 0)} disabled className={inputLockedCls} />
              ) : (
                <input type="text" inputMode="decimal" value={valorIntegral} onChange={(e) => setValorIntegral(e.target.value)} placeholder={t('divida_modal.valor_integral_placeholder')} className={inputCls} />
              )}
              {travado && <p className="text-xs text-gray-600 mt-1">{t('divida_modal.travado_aviso')}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.descricao')}</label>
              <input type="text" value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder={t('divida_modal.descricao_placeholder')} className={inputCls} />
            </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer w-fit">
                ...
                <span className="text-sm text-gray-300">{t('divida_modal.pagar_com_rake')}</span>
              </label>
              <p className="text-xs text-gray-500">
                {tipo === 'acordo' ? t('divida_modal.pagar_com_rake_desc_acordo') : t('divida_modal.pagar_com_rake_desc_simples')}
              </p>
            </div>
            ...
            {simplesComRake && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.rakeback_pct')}</label>
                  <input type="text" inputMode="decimal" value={rakebackPct} onChange={(e) => setRakebackPct(e.target.value)} placeholder={t('divida_modal.rakeback_pct_placeholder')} className={inputCls} />
                  <p className="text-xs text-gray-500 mt-1.5">{t('divida_modal.rakeback_pct_desc')}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.pagamento_minimo')}</label>
                  <input type="text" inputMode="decimal" value={pagamentoMinimo} onChange={(e) => setPagamentoMinimo(e.target.value)} placeholder={t('divida_modal.pagamento_minimo_placeholder_rakeback')} disabled={!rakebackPct} className={rakebackPct ? inputCls : inputLockedCls} />
                  <p className="text-xs text-gray-500 mt-1.5">{t('divida_modal.pagamento_minimo_desc_rakeback')}</p>
                </div>
              </div>
            )}
            ...
            {tipo === 'acordo' && !semCronograma && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.quantidade_parcelas')}<span className="text-gray-500 ml-1">*</span></label>
                    {travado ? (
                  ...
                  <div>
                    ...
                    {travado ? (
                      ...
                      <input type="text" inputMode="decimal" value={pagamentoMinimo} onChange={(e) => setPagamentoMinimo(e.target.value)} placeholder={t('divida_modal.pagamento_minimo_placeholder_parcela')} className={inputCls} />
                    )}
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">{t('divida_modal.data_primeira_parcela')}<span className="text-gray-500 ml-1">*</span></label>
                  {travado ? (
                  ...
                  <p className="text-xs text-gray-500 mt-1.5">{t('divida_modal.data_primeira_parcela_desc')}</p>
                </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-3 cursor-pointer w-fit">
                    ...
                    <span className="text-sm text-gray-300">{t('divida_modal.juros')}</span>
                  </label>
                  {jurosAtivo && (
                    travado ? (
                      <input type="text" value={jurosPct} disabled className={inputLockedCls} />
                    ) : (
                      <input type="text" inputMode="decimal" value={jurosPct} onChange={(e) => setJurosPct(e.target.value)} placeholder={t('divida_modal.juros_placeholder')} className={inputCls} />
                    )
                  )}
                  {jurosAtivo && <p className="text-xs text-gray-500">{t('divida_modal.juros_desc')}</p>}
                </div>

                {preview && (
                  <div className="rounded-lg border border-white/10 bg-surface2 p-3 space-y-2">
                    <p className="text-xs text-gray-400">
                      {jurosAtivo ? t('divida_modal.preview_valor_com_juros') : t('divida_modal.preview_valor_total')}: <span className="text-white font-medium">{fmt(preview.valorComJuros)}</span>
                      {' · '}{t('divida_modal.preview_parcela_base')}: <span className="text-white font-medium">{fmt(preview.valorParcela)}</span>
                      {' · '}{preview.quantidadeParcelasFinal}x
                      {preview.usouPagamentoMinimo && <span className="text-gold"> {t('divida_modal.preview_ajustado_minimo')}</span>}
                    </p>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      {preview.parcelas.map((p) => (
                        <div key={p.numero} className="flex justify-between text-xs text-gray-500">
                          <span>{t('divida_modal.parcela_label', { n: p.numero })} — {new Date(p.vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}</span>
          ...
          <div className="shrink-0 flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10">
            <button type="button" onClick={onClose} className="px-4 py-2 border border-white/10 rounded-lg text-sm text-gray-400 hover:text-white hover:border-white/20 transition-colors">{t('divida_modal.cancelar')}</button>
            <button type="submit" disabled={saving || !podeSalvar} className="flex items-center gap-2 px-5 py-2 bg-gold text-surface rounded-lg text-sm font-semibold hover:bg-gold/90 disabled:opacity-50 transition-colors">
              {saving && <Loader2 size={14} className="animate-spin" />}{t('divida_modal.salvar')}
            </button>
          </div>
        </form>
      </div>
