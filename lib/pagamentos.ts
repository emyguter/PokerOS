...
// "Rollover": isenção de multa negociada pros dois lados — Cássio, em
// áudio: "o rollover é a opção dos dois lados, da negociação optarem por
// rolar dívida pra próxima semana sem nenhum problema... o acerto dele não
// está pendendo, ele não vai ter multa, não tem nada, ele só passou pra
// próxima semana". A Diferença em si NÃO precisa de nenhuma ação pra
// carregar pro próximo Acerto — isso já é automático (ver
// buscarSaldoArrastado em lib/acertos-engine.ts), inclusive a multa quando
// o clube tem Regra cadastrada. Rollover só marca esse período específico
// como isento dessa multa automática — sem criar lançamento nenhum.
export async function rolloverAcerto(acertoId: string): Promise<void> {
  const { error } = await supabase.from('acertos').update({ rollover_sem_multa: true }).eq('id', acertoId)
